...
class Ollama(Model):
    def __init__(self, model_name: str = "qwen2.5-coder:32b-instruct-q4_K_M"):
        logging.debug(f"Initializing Ollama model with model_name: {model_name}")
        super().__init__()
        ...
        logging.debug(f"Ollama model initialized with model_name: {model_name}, api_costs: {self.api_costs}")
    ...
    def prompt(self, prompt_text: str, use_json_schema: bool = False) -> str:
        ...
        logging.debug(f"Sending messages to Ollama: {messages}")
